    ...
    if elevator_no == 0 {
        ...
        if el.wait_at != -1 {
            if el.wait_at == at_floor {
                ...
                /* FILL IN CODE HERE */
                el.is_elevator_ready = true;
                let elevator_signal = &ELEVATOR.1;
                elevator_signal.notify_one();
                ...
                while !el.is_passenger_ready /* FILL IN CORRECT CONDITION HERE */ {
                    /* WAIT FOR CONDITION HERE */
                    let passenger_signal = &ELEVATOR.2;
                    el = passenger_signal.wait(el).unwrap();
                }
